...
class PaddleWindow(Gtk.Window):
    def __init__(self, window_size, x, y, is_ai):
        ...
        if not is_ai:
            ...
            store['hooks']['ball_position'] = self.ball_position_change
    ...
    def tick(self, _widget, _frame_clock):
        ...
        if self.is_ai:
            store['ai_paddle'] = (self.x, current_y)
        else:
            store['player_paddle'] = (self.x, current_y)

        return True
    def check_minimized(self, _widget, event):
        if event.new_window_state & Gdk.WindowState.ICONIFIED:
            self.rebuild()
    def realize(self, _data):
    ...
    def btn_press(self, _widget, event):
        if self.target_y == None:
            ...
            return True
    def btn_release(self, _widget, _event):
